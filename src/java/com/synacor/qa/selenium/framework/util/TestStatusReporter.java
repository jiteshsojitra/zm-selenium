...
public class TestStatusReporter extends TestListenerAdapter {
	...
	private static class GetFromFile {
		...
		private static String getDoubleLineAt(int lineNumber) {
			...
			if (lineNumber > longArray.size()) {
				try {
					// go to the farthest read point of file
	...
	public TestStatusReporter(String atype, ByteArrayOutputStream baos, PrintStream ps) throws Exception {
		path = ConfigProperties.getStringProperty("testOutputDirectory") + "\\"
				+ ConfigProperties.startAttGetVersionString() + "\\" + atype + "\\"
				+ ConfigProperties.getStringProperty("browser") + "\\" + ConfigProperties.getStringProperty("locale");
		...
		output = new PrintWriter(new FileWriter(new File(path + "/startatt-failed.xml")));
	...
	public void onFinish(ITestContext testContext) {
		...
		copyFile(path + "\\" + inProgressDir + "\\class.html",
				path + "\\" + categoryDir + "\\" + testContext.getName() + ".html");
	...
	public void onTestStart(ITestResult tr) {
		...
		if (!(new File(path + "\\" + failDir + "\\" + fullTestName + ".txt").exists())
			...
			System.out.println(
					GetFromFile.getCode(tr.getTestClass().getName().replaceAll("\\.", "/") + ".java", tr.getName()));
		}

		log(new Date().toString() + "-------------- STARTED " + fullTestName + getParameters(tr.getParameters())
				+ " STARTED" + " ----------------------------------------");
		...
		try {
			...
			String testdetails =
					new Date().toString() + " -  Duration " + duration + " sec " + "\n - " + (passed + failed + skipped)
							+ " run " + "\n - " + passed + " pass  " + "\n - " + failed + " fail   - " + confFailed
	...
	private void generateReport() {
		...
		try {
			version = ConfigProperties.startAttGetVersionString();
		...
		String lines = "\n--------------------------------------------\n";
		String uri = (bodyfileXpPath.replace("T:/", "http://tms.lab.synacor.com/testlogs/")).replace("\\", "/");
		...
		body.append("\nPass: " + passed);
		// body.append("\nbrowser: " +
		// ClientSessionFactory.session().currentBrowserName());
	...
	private String getTestList(ArrayList<String> array) {
		...
		for (int i = 0; i < array.size(); i++) {
			...
			// result.append("<a href='" + uri + methodName.replace(".","/") + "'>" +
			// methodName+ "</a>" + "\n");
		...
		try {
			...
			if (testName.indexOf("tests") != -1) {
				...
				if (subPath.indexOf("\\") != -1) {
					...
					// hierachy
					pw = new PrintWriter(new FileWriter(path + "\\" + dir + "\\" + subPath + "\\" + testName + ".txt"),
							true);
					pw.println(baos.toString());
					pw.close();
				}
	...
	public void onConfigurationFailure(ITestResult tr) {
		...
		log(new Date().toString() + "-------------- CONFIGURATION FAILED " + fullTestName
		...
		if (tr.getThrowable() != null) {
			...
			confFailReasonMap.put(fullTestName,
	...
	public void onTestFailure(ITestResult tr) {
		...
		log(new Date().toString() + "-------------- FAILED " + fullTestName
				+ " FAILED ----------------------------------------");

		printToFile(failDir, fullTestName);

		// ClientSessionFactory.session().selenium().captureScreenshot(path + "\\" +
		// failDir + "\\"+ fullTestName + ".png");

		classInProgressPrintWriter.println("<br>\t" + new Date().toString() + " FAILED");
		classInProgressPrintWriter
				.println("<br>\t" + "<a href='../fail/" + fullTestName + ".png' target=newWindow><img src='../fail/"
						+ fullTestName + ".png' width=100 height=100 border=0></a>" + "  <a href='../fail/"
						+ fullTestName + ".txt' target=newWindow> details </a>");

		if ((tr.getThrowable() != null) && (tr.getThrowable().getMessage() != null)) {
			classInProgressPrintWriter.println("<br>\t <pre>"
					+ tr.getThrowable().getMessage().replaceAll("<", "&lt;").replaceAll(">", "&gt;") + "</pre>");
		}
		classInProgressPrintWriter.println("<br>\t" + getErrorLine(tr.getThrowable(), tr.getName()));
		classInProgressPrintWriter.flush();

		failReasonMap.put(fullTestName,
				tr.getThrowable().getMessage() + "\n\t" + getErrorLine(tr.getThrowable(), tr.getName()));
	...
	private String getErrorLine(Throwable thr, String methodName) {
		...
		for (int i = 0; i < seArray.length; i++) {
			...
			if (ste.getMethodName().equals(methodName)) {
				result = "at " + ste.getClassName() + "." + methodName + " (" + ste.getFileName() + ":"
						+ ste.getLineNumber() + ")";
				result += "<br>\n\t" + GetFromFile.getDoubleLineAt(ste.getClassName().replaceAll("\\.", "/") + ".java",
						ste.getLineNumber());
				break;
			}
		}
	...
	public void onConfigurationSkip(ITestResult tr) {
		...
		log(new Date().toString() + "-------------- CONFIGURATION  SKIPPED " + fullTestName
				+ "  CONFIGURATION SKIPPED ----------------------------------------");
		confSkipArray.add(fullTestName);

		printToFile(confSkipDir, fullTestName);
		// ClientSessionFactory.session().selenium().captureScreenshot(path + "\\" +
		// skipDir + "\\"+ fullTestName + ".png");
		...
		if (tr.getThrowable() != null) {
			...
			confSkipReasonMap.put(fullTestName,
					tr.getThrowable().getMessage() + "\n\t" + getErrorLine(tr.getThrowable(), tr.getName()));
		}
		classInProgressPrintWriter.flush();
	...
	public void onTestSkipped(ITestResult tr) {
		...
		log(new Date().toString() + "-------------- SKIPPED " + fullTestName
				+ " SKIPPED ----------------------------------------");
		...
		if (tr.getThrowable() != null) {
			...
			skipReasonMap.put(fullTestName,
					tr.getThrowable().getMessage() + "\n\t" + getErrorLine(tr.getThrowable(), tr.getName()));
		}
	...
	public void onTestSuccess(ITestResult tr) {
		String fullTestName = tr.getTestClass().getName() + "." + tr.getName();
		log(new Date().toString() + "-------------- PASSED " + fullTestName
				+ " PASSED ----------------------------------------");
